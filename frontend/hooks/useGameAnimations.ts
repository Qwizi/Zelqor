  ...
  useEffect(() => {
    ...
    for (const e of newEvents) {
      if (e.type === "troops_sent") {
        const eventKey = [
          e.action_type, e.player_id, e.source_region_id,
          e.target_region_id, e.unit_type, e.units,
        ].join(":");
        ...
        for (let i = 0; i < rocketsThrough; i++) {
          ...
          newAnims.push({
            ...
            units: rocketDmg, unitCount: rocketDmg,
            unitType: "artillery", type: "attack" as const,
            startTime: Date.now() + delay, durationMs: ROCKET_FLIGHT_MS, playerId,
          });
        ...
        if (interceptedCount > 0 && samRegionIds.length > 0) {
          for (let i = 0; i < interceptedCount; i++) {
            ...
            newAnims.push({
              id: artAnimId, sourceId, targetId, color,
              units: rocketDmg, unitCount: rocketDmg,
              unitType: "artillery", type: "attack" as const,
              startTime: Date.now() + interceptDelay, durationMs: ROCKET_FLIGHT_MS, playerId,
            });
            ...
            setTimeout(() => {
              window.dispatchEvent(new CustomEvent("sam-intercept-visual", {
                detail: { sourceId, targetId, samRegionId: capturedSamRegion, flightMs: ROCKET_FLIGHT_MS, samFlightMs: SAM_FLIGHT_MS },
              }));
            }, interceptDelay);
          ...
          if (sourceId && targetId) {
            ...
            for (let i = 0; i < numReturn; i++) {
              newAnims.push({
                ...
                sourceId: targetId, targetId: sourceId, color: intColor,
                units: 1, unitCount: 1, unitType: "fighter",
                type: "move" as const, startTime: Date.now(), durationMs: 3000,
                playerId: interceptorPlayerId, pathOffset: offsets[i],
              });
            }
          }
        }
          ...
          if (sourceId) {
            ...
            for (let ei = 0; ei < numEscorts; ei++) {
              newAnims.push({
                ...
                sourceId: targetId, targetId: sourceId, color,
                units: 1, unitCount: 1, unitType: "fighter",
                type: "move" as const, startTime: Date.now(), durationMs: 4000,
                playerId, pathOffset: offsets[ei],
              });
            }
          }
        }
