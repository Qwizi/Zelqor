...
class MockAudio {
  ...
  play() { return MockAudio.playMock() }
...
import { useAudio, SOUNDS, MUSIC_TRACKS } from '../useAudio'
...
describe('useAudio', () => {
  ...
  it('playSound() respects sound cooldown — second call within 80ms is ignored', () => {
    ...
    const countBefore = MockAudio.instances.length
    act(() => { result.current.playSound('click') })
    act(() => { result.current.playSound('click') }) // immediately again

    // Only one new Audio instance for 'click' should be created
    const clickInstances = MockAudio.instances
      .slice(countBefore)
      .filter((a) => a.src === SOUNDS.click)
  ...
  it('playSound() sets volume to 0.55', () => {
    ...
    act(() => { result.current.playSound('build') })
  ...
  it('startMusic() sets music volume to 0.10', () => {
    ...
    expect(musicAudio.volume).toBe(0.10)
  })
  ...
  it('selectTrack() ignores out-of-range indices', () => {
    ...
    act(() => { result.current.selectTrack(9999) })
