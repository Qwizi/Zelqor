...
describe('ActionBar', () => {
  ...
  it('calls onSelectedUnitTypeChange when a unit type button is clicked', () => {
    ...
    render(
      <ActionBar
        {...defaultProps({ sourceRegion, onSelectedUnitTypeChange })}
      />
    )
  ...
  it('confirm button is disabled when targets list is empty', () => {
    ...
    const confirmButtons = screen.getAllByRole('button').filter(
      (btn) => btn.textContent?.includes('Ruch') || btn.textContent?.includes('Atak')
    )
  ...
  it('confirm button is enabled when targets exist', () => {
    render(<ActionBar {...defaultProps({ targets: [MOVE_TARGET] })} />)
    const confirmButtons = screen.getAllByRole('button').filter(
      (btn) => btn.textContent?.includes('Ruch')
    )
  ...
  it('calls onConfirm with correct payload when confirm button clicked', () => {
    ...
    render(
      <ActionBar
        {...defaultProps({ targets: [MOVE_TARGET], onConfirm })}
      />
    )
    const confirmButtons = screen.getAllByRole('button').filter(
      (btn) => btn.textContent?.includes('Ruch')
    ...
    expect(onConfirm).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        allocations: expect.arrayContaining([
          expect.objectContaining({ regionId: 'r-target' }),
        ]),
      })
    )
  })
  ...
  it('calls onRemoveTarget when a target chip is clicked', () => {
    ...
    render(
      <ActionBar
        {...defaultProps({ targets: [MOVE_TARGET], onRemoveTarget })}
      />
    )
  ...
  it('applies attack border styling when any target is an attack', () => {
    const { container } = render(
      <ActionBar {...defaultProps({ targets: [ATTACK_TARGET] })} />
    )
  ...
  it('applies move border styling when all targets are moves', () => {
    const { container } = render(
      <ActionBar {...defaultProps({ targets: [MOVE_TARGET] })} />
    )
  ...
  it('renders correct Polish label for ship unit type', () => {
    ...
    render(
      <ActionBar
        {...defaultProps({ sourceRegion, selectedUnitType: 'ship' })}
      />
    )
  ...
  it('renders correct Polish label for fighter unit type', () => {
    ...
    render(
      <ActionBar
        {...defaultProps({ sourceRegion, selectedUnitType: 'fighter' })}
      />
    )
