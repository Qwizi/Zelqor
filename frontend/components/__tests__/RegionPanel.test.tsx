...
import type { GamePlayer, GameRegion } from "@/hooks/useGameSocket";
...
function makePlayers(_ownerId = MY_USER_ID): Record<string, GamePlayer> {
  return {
    [MY_USER_ID]: {
      user_id: MY_USER_ID,
      username: "MyPlayer",
...
describe("RegionPanel", () => {
  ...
  it("calls onBuild when build button clicked with sufficient energy", () => {
    ...
    const barracksBtn = barracksButtons.find((btn) => btn.textContent?.includes("Barracks"));
  ...
  it("disables build button when energy is insufficient", () => {
    ...
    const barracksButtons = screen.getAllByRole("button");
    const barracksBtn = barracksButtons.find((btn) => btn.textContent?.includes("Barracks") && btn.closest("button"));
    expect(barracksBtn).toBeDefined();
  ...
  it("calls onProduceUnit when unit button is clicked", () => {
    ...
    render(<RegionPanel {...defaultProps({ region, onProduceUnit, myEnergy: 100 })} />);
    fireEvent.click(screen.getByText("Jednostki"));

    const infantryButtons = screen.getAllByRole("button");
    const infantryBtn = infantryButtons.find((btn) => btn.textContent?.includes("Infantry"));
  ...
  it("shows queued buildings in info tab", () => {
    const buildingQueue = [{ region_id: "r-warsaw", building_type: "barracks", queued_at: 1 }];
