...
function defaultProps(overrides: Partial<Parameters<typeof AbilityBar>[0]> = {}) {
...
describe("AbilityBar", () => {
  ...
  it("renders nothing when abilities list is empty", () => {
    const { container } = render(<AbilityBar {...defaultProps({ abilities: [], abilityScrolls: {} })} />);
    expect(container.firstChild).toBeNull();
  });

  it("renders nothing when abilityScrolls is undefined", () => {
    const { container } = render(<AbilityBar {...defaultProps({ abilityScrolls: undefined })} />);
    expect(container.firstChild).toBeNull();
  });

  it("renders nothing when abilityScrolls is empty (no uses remaining)", () => {
    const { container } = render(<AbilityBar {...defaultProps({ abilityScrolls: {} })} />);
    expect(container.firstChild).toBeNull();
  });
  ...
  it("disables ability button when on cooldown", () => {
    ...
    const abilityButtons = buttons.filter((btn) => btn.title?.includes("Strike"));
  ...
  it("calls onActivateBoost instead of onSelectAbility for boost abilities", () => {
    ...
    const buttons = screen.getAllByRole("button");
    const boostButtons = buttons.filter((btn) => btn.title?.includes("Production Boost"));
  ...
  it("disables abilities not matching allowedAbility in tutorial mode", () => {
    ...
    const buttons = screen.getAllByRole("button");
    const strikeButtons = buttons.filter((btn) => btn.title?.includes("Strike"));
  ...
  it("button title contains remaining uses when defined", () => {
    render(<AbilityBar {...defaultProps({ abilityScrolls: { strike: 7 } })} />);
    const buttons = screen.getAllByRole("button");
    const abilityButtons = buttons.filter((btn) => btn.title?.includes("Strike"));
