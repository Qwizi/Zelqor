    ...
    <div className="animate-page-in space-y-3 md:space-y-8 -mx-4 md:mx-0 -mt-2 md:mt-0">
          ...
          (() => {
            ...
            const platforms = [
              ...
              },
              {
                ...
                icon: (
                  <svg viewBox="0 0 24 24" className="h-4 w-4" fill="currentColor">
                    <path d="M9.101 23.691v-7.98H6.627v-3.667h2.474v-1.58c0-4.085 1.848-5.978 5.858-5.978.401 0 1.09.07 1.373.14v3.324c-.149-.016-.41-.023-.733-.023-1.04 0-1.443.394-1.443 1.418v2.7h4.057l-.695 3.666h-3.362v8.127C19.396 23.145 24 18.07 24 12A12 12 0 1 0 9.101 23.691" />
                  </svg>
                ),
              },
              {
      ...
      <div className="animate-stagger flex gap-2.5 overflow-x-auto px-4 pb-1 md:px-0 md:grid md:grid-cols-2 lg:grid-cols-4 md:gap-4 md:overflow-visible scrollbar-none [-ms-overflow-style:none] [scrollbar-width:none]">
        <div className="hover-lift flex shrink-0 items-center gap-2.5 rounded-2xl bg-card/60 md:bg-card border border-transparent md:border-border px-3.5 py-3 md:p-5 md:flex-col md:items-start md:gap-2 min-w-[120px] md:min-w-0">
          ...
          <span className="text-[10px] md:text-xs uppercase tracking-[0.15em] text-muted-foreground font-medium">
            Status
          </span>
        ...
        {winner && (
          <div className="hover-lift flex shrink-0 items-center gap-2.5 rounded-2xl bg-accent/5 md:bg-card border border-accent/20 md:border-accent/25 px-3.5 py-3 md:p-5 md:flex-col md:items-start md:gap-2 min-w-[120px] md:min-w-0">
            ...
            <span className="text-[10px] md:text-xs uppercase tracking-[0.15em] text-accent/70 font-medium">
              Zwycięzca
            </span>
      ...
      {(match.started_at || match.finished_at) && (
        <div className="flex flex-wrap gap-3 md:gap-6 text-xs md:text-base text-muted-foreground px-4 md:px-0">
          ...
          {result && (
            <span>
              Ticki: <span className="text-foreground">{result.total_ticks}</span>
            </span>
          )}
        </div>
      )}
      ...
      {/* MVP + ELO + Stats/min */}
      {result &&
        result.player_results.length > 0 &&
        (() => {
            ...
            <div className="px-4 md:px-0 space-y-3 md:space-y-4">
              ...
              <div className="flex items-center gap-3 md:gap-4 rounded-2xl border border-accent/20 bg-accent/5 p-3 md:p-4">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    {mvpPlayer && (
                      <div
                        ...
                        style={{ backgroundColor: mvpPlayer.color }}
                      />
                    )}
                ...
                <div className="flex gap-3 md:gap-4 shrink-0 text-center">
                  <div>
                    <div className="font-display text-base md:text-lg text-primary tabular-nums">
                      {mvp.regions_conquered}
                    </div>
                    <div className="text-[9px] md:text-[10px] text-muted-foreground uppercase">Regiony</div>
                  </div>
                  <div>
                    <div className="font-display text-base md:text-lg text-foreground tabular-nums">
                      {mvp.units_produced}
                    </div>
                    <div className="text-[9px] md:text-[10px] text-muted-foreground uppercase">Jednostki</div>
                  </div>
                  <div>
                    <div className="font-display text-base md:text-lg text-accent tabular-nums">
                      {mvp.buildings_built}
                    </div>
                    ...
                    {result.player_results.map((pr) => {
                      ...
                      return (
                        <div key={pr.user_id} className="flex items-center gap-2.5">
                          ...
                          <span
                            className={`font-display text-base md:text-lg tabular-nums font-semibold flex items-center gap-1 ${
                              pr.elo_change > 0
                                ? "text-green-400"
                                : pr.elo_change < 0
                                  ? "text-destructive"
                ...
                {durationMin > 0 && (
                  <div className="rounded-2xl border border-border bg-card/50 md:bg-card p-3 md:p-4">
                    <div className="flex items-center gap-1.5 mb-2 md:mb-3">
                      ...
                      <p className="text-[10px] md:text-xs uppercase tracking-[0.15em] text-muted-foreground font-medium">
                        Tempo gry (na minutę)
                      </p>
                    </div>
                    <div className="space-y-2">
                          ...
                          <div key={pr.user_id} className="flex items-center gap-2.5">
                            ...
                            <div className="flex gap-3 shrink-0 text-xs md:text-sm tabular-nums">
                              <span className="text-primary">
                                {regPerMin} <span className="text-muted-foreground text-[10px]">reg</span>
                              </span>
                              <span className="text-foreground">
                                {unitsPerMin} <span className="text-muted-foreground text-[10px]">jedn</span>
                              </span>
                            </div>
                          </div>
                        );
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    {!player.is_alive && (
                      <span className="text-[10px] text-destructive font-medium">Wyeliminowany</span>
                    )}
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell className="py-5 text-center font-display text-2xl text-foreground">
                      {pr ? (match.players.length > 2 ? `#${pr.placement}` : isWinner ? "🏆" : "💀") : "—"}
                    </TableCell>
