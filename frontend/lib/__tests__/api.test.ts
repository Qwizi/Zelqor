...
import { APIError, BannedError, login, register, getConfig, getLevelStat } from '../api'
...
describe('fetchAPI via login()', () => {
  ...
  it('makes a POST request to /token/pair with credentials', async () => {
    ...
    mockFetch.mockResolvedValueOnce(
      makeResponse({ access: 'acc', refresh: 'ref' })
    )
  ...
  it('returns a TokenPair on success', async () => {
    vi.mocked(fetch).mockResolvedValueOnce(
      makeResponse({ access: 'a', refresh: 'r' })
    )
  ...
  it('includes Authorization header when token option is provided', async () => {
    ...
    vi.mocked(fetch).mockResolvedValueOnce(
      makeResponse({ id: '1', username: 'tester', email: 'a@b.com', role: 'player', elo_rating: 1200, date_joined: '', tutorial_completed: false, is_banned: false })
    )
