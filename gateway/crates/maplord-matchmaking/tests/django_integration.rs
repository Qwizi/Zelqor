...
//!   cargo test -p maplord-matchmaking --features testing \
//!   -- --include-ignored
//! ```
...
fn drain_all(
    rx: &mut tokio::sync::mpsc::UnboundedReceiver<maplord_matchmaking::MatchmakingMessage>,
) -> (Vec<serde_json::Value>, bool) {
    let mut out = Vec::new();
    let mut saw_close = false;
    while let Ok(msg) = rx.try_recv() {
        match msg {
            maplord_matchmaking::MatchmakingMessage::Json(v) => out.push(v),
            maplord_matchmaking::MatchmakingMessage::Close => saw_close = true,
        }
    }
    (out, saw_close)
}
