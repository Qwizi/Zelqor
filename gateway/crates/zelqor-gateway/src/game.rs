...
use dashmap::DashMap;
use zelqor_ai::{BotBrain, BotStrategy, TutorialBotBrain};
use zelqor_engine::{
...
use zelqor_state::{FullGameState, GameStateManager};
...
use tokio::sync::mpsc;
use zelqor_anticheat::{AnticheatEngine, AnticheatVerdict};
...
async fn compute_weather_from_meta(state_mgr: &GameStateManager, now_secs: i64) -> zelqor_engine::WeatherState {
    ...
    zelqor_engine::compute_weather_with_flags(now_secs, weather_enabled, day_night_enabled)
}
    ...
    loop {
        ...
        if !has_alive_human {
            ...
            let early_now_secs = SystemTime::now()
                ...
                .unwrap_or_default()
                .as_secs() as i64;
            let early_weather = zelqor_engine::compute_weather_with_settings(early_now_secs, &settings);
        ...
        match &ac_verdict {
            ...
            AnticheatVerdict::FlagPlayer { player_id, reason } => {
                ...
                let cheat_now_secs = SystemTime::now()
                    ...
                    .unwrap_or_default()
                    .as_secs() as i64;
                let cheat_weather = zelqor_engine::compute_weather_with_settings(cheat_now_secs, &settings);
                let flag_msg = json!({
                    "type": "game_tick",
                    "tick": tick,
                    "events": [{
        ...
        let now_secs = std::time::SystemTime::now()
            ...
            .unwrap_or_default()
            .as_secs() as i64;
        let weather = zelqor_engine::compute_weather_with_settings(now_secs, &settings);
        ...
        if is_tutorial && current_tick_multiplier > 1 && !primary_game_over {
            for _ in 1..current_tick_multiplier {
                ...
                let extra_now_secs = std::time::SystemTime::now()
                    ...
                    .unwrap_or_default()
                    .as_secs() as i64;
                let extra_weather = zelqor_engine::compute_weather_with_settings(extra_now_secs, &settings);
...
mod tests {
    use super::*;
    use zelqor_engine::{Event, Player, Region};
