...
class ClanGlobalController:
    ...
    def leaderboard(self, request, sort: str = "elo", limit: int = 50, offset: int = 0):
        ...
        qs = qs.order_by(order, "-created_at") if order else qs.order_by("-created_at")
    ...
    def accept_invitation(self, request, invitation_id: uuid.UUID):
        inv = (
            ClanInvitation.objects.select_related("clan")
            .filter(
                pk=invitation_id,
                invited_user=request.auth,
                status=ClanInvitation.Status.PENDING,
        ...
        with transaction.atomic():
            ...
            ClanMembership.objects.create(
                clan=clan,
                user=request.auth,
                role=ClanMembership.Role.RECRUIT,
            )
    ...
    def accept_join_request(self, request, request_id: uuid.UUID):
        jr = (
            ClanJoinRequest.objects.select_related("clan", "user")
            .filter(
                pk=request_id,
                status=ClanJoinRequest.Status.PENDING,
            )
            .first()
        )
        if not jr:
    ...
    def decline_join_request(self, request, request_id: uuid.UUID):
        jr = (
            ClanJoinRequest.objects.select_related("clan")
            .filter(
                pk=request_id,
                status=ClanJoinRequest.Status.PENDING,
            )
    ...
    def accept_war(self, request, war_id: uuid.UUID):
        war = (
            ClanWar.objects.select_related("challenger", "defender")
            .filter(
                pk=war_id,
    ...
    def get_war(self, request, war_id: uuid.UUID):
        ...
        war = (
            ClanWar.objects.select_related(
                "challenger",
                "defender",
                "winner",
            )
    ...
    def create_clan(self, request, payload: ClanCreateSchema):
        ...
        with transaction.atomic():
            ...
            if wallet.gold < CLAN_CREATION_COST:
                raise HttpError(
                    400, f"Niewystarczająca ilość złota. Utworzenie klanu kosztuje {CLAN_CREATION_COST} złota."
                )
