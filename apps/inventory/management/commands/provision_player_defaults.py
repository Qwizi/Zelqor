...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for user in users:
            ...
            if old_default:
                old_item_slugs = set(DeckItem.objects.filter(deck=old_default).values_list("item__slug", flat=True))
            ...
            for slug, item in items.items():
                DeckItem.objects.get_or_create(
                    deck=deck, item=item, defaults={"quantity": 1, "instance": instance_map.get(slug)}
                )
