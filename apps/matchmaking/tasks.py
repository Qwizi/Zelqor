...
def cleanup_stale_lobbies():
    ...
    for lobby in expired_full:
        ...
        if kick_count > 0:
            ...
            logger.info(f"Lobby {lobby.id}: kicked {kick_count} unready player(s), reverted to waiting")
    ...
    # 3. Empty lobbies
    empty_lobbies = (
        Lobby.objects.filter(status__in=[Lobby.Status.WAITING, Lobby.Status.FULL, Lobby.Status.READY])
