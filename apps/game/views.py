...
class GameController:
    @route.get(
        "/results/{match_id}/", response=MatchResultOutSchema, auth=ActiveUserJWTAuth(), permissions=[IsAuthenticated]
    )
    ...
    @route.get(
        "/snapshots/{match_id}/",
        response=list[SnapshotTickSchema],
    ...
    def list_snapshots(self, request, match_id: str):
        ...
        return list(GameStateSnapshot.objects.filter(match_id=match_id).order_by("tick").values("tick", "created_at"))

    @route.get(
        "/snapshots/{match_id}/{tick}/",
        response=SnapshotDetailSchema,
        auth=ActiveUserJWTAuth(),
        permissions=[IsAuthenticated],
    )
...
class ShareController:
    ...
    def _get_match_result_data(self, match_id) -> dict:
        ...
        try:
            match_result = MatchResult.objects.prefetch_related("player_results", "player_results__user").get(
                match_id=match_id
            )
            result_data = {
        ...
        snapshot_ticks = list(
            GameStateSnapshot.objects.filter(match_id=match_id).order_by("tick").values_list("tick", flat=True)
        )
