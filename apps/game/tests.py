...
class GameModelStrTests(TestCase):
    ...
    def test_snapshot_str(self):
        snap = GameStateSnapshot.objects.create(match=self.match, tick=42, state_data={"foo": "bar"})
...
class GameInternalAPITests(TestCase):
    ...
    def test_save_snapshot_persists_to_db(self):
        ...
        self.assertTrue(GameStateSnapshot.objects.filter(match=self.match, tick=20).exists())
    ...
    def test_report_violation_persists_all_fields(self):
        ...
        v = AnticheatViolation.objects.get(match=self.match, player=self.user1, violation_kind="impossible_timing")
