# Zelqor TWA (Trusted Web Activity)

Android wrapper for Zelqor — publishes the web app on Google Play.
...
```bash
keytool -genkey -v -keystore zelqor.keystore -alias zelqor -keyalg RSA -keysize 2048 -validity 10000
```

### 2. Get SHA-256 fingerprint

```bash
keytool -list -v -keystore zelqor.keystore -alias zelqor | grep SHA256
```
...
This file must be served at `https://zelqor.qwizi.ovh/.well-known/assetlinks.json` for Chrome to trust the TWA and hide the URL bar.
...
```bash
apksigner sign --ks zelqor.keystore --ks-key-alias zelqor app/build/outputs/apk/release/app-release-unsigned.apk
```
...
- The app opens `https://zelqor.qwizi.ovh/dashboard` in Chrome without the URL bar
